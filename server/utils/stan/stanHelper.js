...
import { connect as _connect } from "node-nats-streaming";
...
export function getInstance(server, cluster, appName, logger = LOGGER) {
...
class Nerve {
  ...
  connect() {
    ...
    return new Promise((resolve, reject) => {
      const conn = _connect(this.cluster, this.clientId, {
        url: this.server,
        reconnect: true,
